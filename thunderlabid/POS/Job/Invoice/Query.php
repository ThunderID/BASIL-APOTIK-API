...
namespace Thunderlabid\POS\Job\Invoice;
...
class Query
{
	...
	protected function buildQuery(Builder $q)
	{
		...
		foreach ($this->queries as $field => $v)    
		{
			if (isset($v) && !is_null($v))
			{
				...
				{
					/*----------  Current Model  ----------*/
					case 'pos_point_id': $q = $q->where('pos_point_id', $v); break;
                    ...
                    case 'has_cancelled':   
                        if($v){
                            $q = $q->wherenotnull('cancelled_at'); 
                        }else{
                            $q = $q->wherenull('cancelled_at'); 
                        }
                    break;
	...
	public function handle(Invoice $model)
	{
		$q = $model->newQuery();
		$q = $this->buildQuery($q);
		if ($this->is_paginate)
